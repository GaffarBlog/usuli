                    ...
                    <ol class="flex flex-col">
                        @foreach ($popularPosts as $index => $post)
                            <li class="flex items-baseline gap-5 border-b border-hairline py-5 first:pt-1">
                                ...
                                <a href="{{ route('blog.show', $post->slug) }}" class="font-serif text-[1.2rem] font-medium leading-normal text-ink transition-colors duration-300 hover:text-brand-deep">{{ $post->title }}</a>
                            </li>
                        @endforeach
                    </ol>
                </div>
        ...
        <div class="shell">
            <div class="reveal relative overflow-hidden rounded-[14px] border border-brand-line bg-brand-soft px-[clamp(24px,5vw,40px)] py-[clamp(40px,6vw,72px)] text-center">
                ...
                <form id="newsletterForm" class="mx-auto flex max-w-[520px] gap-3 max-[620px]:flex-col" novalidate>
                    ...
                    <button type="submit" class="whitespace-nowrap rounded-full bg-brand px-7 py-3.5 text-base font-semibold text-white transition-all duration-300 hover:-translate-y-px hover:bg-brand-deep max-[620px]:w-full max-[620px]:py-[15px]">
                        সাবস্ক্রাইব করুন
                    </button>
                </form>
            </div>
