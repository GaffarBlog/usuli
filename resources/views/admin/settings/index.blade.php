            ...
            <form method="POST" action="{{ route('admin.settings.update', ['tab' => 'site']) }}" enctype="multipart/form-data" class="space-y-6 rounded-xl border border-hairline bg-white p-6">
                ...
                <div>
                    ...
                    <textarea id="site_description" name="site_description" rows="3"
                        class="w-full rounded-lg border border-hairline bg-gray-50/50 px-4 py-2.5 text-sm text-ink outline-none transition-colors focus:border-brand focus:bg-white focus:ring-1 focus:ring-brand/20" placeholder="সাইট সম্পর্কে সংক্ষিপ্ত বিবরণ">{{ old('site_description', $settings['site_description'] ?? '') }}</textarea>
                ...
                <div>
                    ...
                    <textarea id="address" name="address" rows="2" class="w-full rounded-lg border border-hairline bg-gray-50/50 px-4 py-2.5 text-sm text-ink outline-none transition-colors focus:border-brand focus:bg-white focus:ring-1 focus:ring-brand/20"
        ...
        @elseif ($activeTab === 'navbar')
            <div class="space-y-6 rounded-xl border border-hairline bg-white p-6">
                <div class="flex items-center justify-between">
                    ...
                    <button type="button" id="addCategoryBtn" class="inline-flex items-center gap-2 rounded-lg bg-brand px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-brand-deep">
                        <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                ...
                <form id="navbarForm" method="POST" action="{{ route('admin.settings.update', ['tab' => 'navbar']) }}">
                    ...
                    <ul id="navbarList" class="space-y-2">
                        @forelse ($navbarItems as $index => $item)
                            <li class="navbar-item flex items-center gap-3 rounded-lg border border-hairline bg-white px-4 py-3 transition-colors hover:border-brand/40" data-type="{{ $item['type'] ?? 'category' }}"
                                data-category-id="{{ $item['category_id'] ?? '' }}" data-index="{{ $index }}">
                                {{-- Drag Handle --}}
                                <span class="sortable-handle cursor-grab text-faint hover:text-ink">
                                    <svg class="h-5 w-5" viewBox="0 0 24 24" fill="currentColor">
                                        <path d="M8 6a2 2 0 1 1 0 4 2 2 0 0 1 0-4zm0 6a2 2 0 1 1 0 4 2 2 0 0 1 0-4zm0 6a2 2 0 1 1 0 4 2 2 0 0 1 0-4zm8-14a2 2 0 1 1 0 4 2 2 0 0 1 0-4zm0 6a2 2 0 1 1 0 4 2 2 0 0 1 0-4zm0 6a2 2 0 1 1 0 4 2 2 0 0 1 0-4z" />
                                    </svg>
                                </span>
                                ...
                                {{-- Label Input --}}
                                <input type="text"
                                    class="navbar-label flex-1 rounded-md border border-hairline bg-gray-50/50 px-3 py-1.5 text-sm text-ink outline-none transition-colors focus:border-brand focus:bg-white focus:ring-1 focus:ring-brand/20"
                                    value="{{ $item['label'] ?? '' }}" placeholder="মেনু টেক্সট">

                                {{-- Toggle (about & contact only) --}}
                                @if (in_array($item['type'] ?? '', ['about', 'contact']))
                                    <label class="relative inline-flex shrink-0 cursor-pointer items-center">
                                        <input type="checkbox" class="navbar-enabled sr-only peer" {{ $item['enabled'] ?? true ? 'checked' : '' }}>
                                        <div
                                            class="h-5 w-9 rounded-full bg-gray-200 after:absolute after:left-[2px] after:top-[2px] after:h-4 after:w-4 after:rounded-full after:bg-white after:transition-all peer-checked:bg-brand peer-checked:after:translate-x-full">
                                        </div>
                                    </label>
                                @endif
                                ...
                                @if (!in_array($item['type'] ?? '', ['home', 'blog', 'about', 'contact']))
                                    <button type="button" class="remove-navbar-item shrink-0 rounded-lg p-1.5 text-faint transition-colors hover:bg-red-50 hover:text-red-500" title="মুছে ফেলুন">
                                        <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                            <path d="M18 6 6 18M6 6l12 12" />
                                        </svg>
                                    </button>
            ...
            <form method="POST" action="{{ route('admin.settings.update', ['tab' => 'about']) }}" class="space-y-6 rounded-xl border border-hairline bg-white p-6">
                ...
                <div class="rounded-lg border border-hairline bg-gray-50/50 p-5 space-y-4">
                    ...
                    <div>
                        ...
                        <textarea id="about_hero_subtitle" name="about_hero_subtitle" rows="2" class="w-full rounded-lg border border-hairline bg-white px-4 py-2.5 text-sm text-ink outline-none transition-colors focus:border-brand focus:ring-1 focus:ring-brand/20"
                ...
                <div class="rounded-lg border border-hairline bg-gray-50/50 p-5 space-y-4">
                    ...
                    <div>
                        ...
                        <textarea id="about_mission_p1" name="about_mission_p1" rows="3" class="w-full rounded-lg border border-hairline bg-white px-4 py-2.5 text-sm text-ink outline-none transition-colors focus:border-brand focus:ring-1 focus:ring-brand/20"
                    ...
                    <div>
                        ...
                        <textarea id="about_mission_p2" name="about_mission_p2" rows="3" class="w-full rounded-lg border border-hairline bg-white px-4 py-2.5 text-sm text-ink outline-none transition-colors focus:border-brand focus:ring-1 focus:ring-brand/20"
                ...
                <div class="rounded-lg border border-hairline bg-gray-50/50 p-5 space-y-4">
                    <div class="flex items-center justify-between">
                        ...
                        <button type="button" id="addValueBtn" class="inline-flex items-center gap-2 rounded-lg bg-brand px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-brand-deep">
                            <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                <path d="M12 5v14M5 12h14" />
                            </svg>
                            মূল্যবোধ যোগ করুন
                            ...
                            <li class="value-item rounded-lg border border-hairline bg-white px-4 py-4 transition-colors hover:border-brand/40" data-index="{{ $index }}">
                                <div class="grid gap-3 sm:grid-cols-2">
                                    <div>
                                        ...
                                        <input type="text"
                                            class="value-title w-full rounded-md border border-hairline bg-gray-50/50 px-3 py-1.5 text-sm text-ink outline-none transition-colors focus:border-brand focus:bg-white focus:ring-1 focus:ring-brand/20"
                                ...
                                <div class="mt-3">
                                    ...
                                    <textarea class="value-description w-full rounded-md border border-hairline bg-gray-50/50 px-3 py-1.5 text-sm text-ink outline-none transition-colors focus:border-brand focus:bg-white focus:ring-1 focus:ring-brand/20" rows="2"
                                        placeholder="মূল্যবোধের বিবরণ">{{ $value['description'] ?? '' }}</textarea>
                                </div>
                ...
                <div class="rounded-lg border border-hairline bg-gray-50/50 p-5 space-y-4">
                    ...
                    <div>
                        ...
                        <textarea id="about_story_p1" name="about_story_p1" rows="3" class="w-full rounded-lg border border-hairline bg-white px-4 py-2.5 text-sm text-ink outline-none transition-colors focus:border-brand focus:ring-1 focus:ring-brand/20"
                    ...
                    <div>
                        ...
                        <textarea id="about_story_p2" name="about_story_p2" rows="3" class="w-full rounded-lg border border-hairline bg-white px-4 py-2.5 text-sm text-ink outline-none transition-colors focus:border-brand focus:ring-1 focus:ring-brand/20"
                    ...
                    <div>
                        ...
                        <textarea id="about_story_p3" name="about_story_p3" rows="3" class="w-full rounded-lg border border-hairline bg-white px-4 py-2.5 text-sm text-ink outline-none transition-colors focus:border-brand focus:ring-1 focus:ring-brand/20"
                ...
                <div class="rounded-lg border border-hairline bg-gray-50/50 p-5 space-y-4">
                    ...
                    <div>
                        ...
                        <textarea id="about_cta_subtitle" name="about_cta_subtitle" rows="2" class="w-full rounded-lg border border-hairline bg-white px-4 py-2.5 text-sm text-ink outline-none transition-colors focus:border-brand focus:ring-1 focus:ring-brand/20"
                ...
                <div class="grid gap-5 sm:grid-cols-2">
                    ...
                    <div>
                        <label for="social_twitter" class="mb-1.5 flex items-center gap-2 text-sm font-medium text-ink">
                            <svg class="h-4 w-4 text-ink" viewBox="0 0 24 24" fill="currentColor">
                    ...
                    <div>
                        <label for="social_instagram" class="mb-1.5 flex items-center gap-2 text-sm font-medium text-ink">
                            <svg class="h-4 w-4 text-[#e4405f]" viewBox="0 0 24 24" fill="currentColor">
                                <path
                    ...
                    <div>
                        <label for="social_youtube" class="mb-1.5 flex items-center gap-2 text-sm font-medium text-ink">
                            <svg class="h-4 w-4 text-[#ff0000]" viewBox="0 0 24 24" fill="currentColor">
                                <path
                    ...
                    <div>
                        <label for="social_telegram" class="mb-1.5 flex items-center gap-2 text-sm font-medium text-ink">
                            <svg class="h-4 w-4 text-[#26a5e4]" viewBox="0 0 24 24" fill="currentColor">
                                <path
                ...
                <form id="footerForm" method="POST" action="{{ route('admin.settings.update', ['tab' => 'footer']) }}">
                    ...
                    <div>
                        ...
                        <input type="text" id="footer_slogan" name="footer_slogan" value="{{ old('footer_slogan', $settings['footer_slogan'] ?? '') }}"
                            class="w-full rounded-lg border border-hairline bg-gray-50/50 px-4 py-2.5 text-sm text-ink outline-none transition-colors focus:border-brand focus:bg-white focus:ring-1 focus:ring-brand/20"
                            placeholder="ফুটারের স্লোগান লিখুন">
                        @error('footer_slogan')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mt-6">
                        <div class="flex items-center justify-between">
                            ...
                            <button type="button" id="addFooterLinkBtn" class="inline-flex items-center gap-2 rounded-lg bg-brand px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-brand-deep">
                                <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                    <path d="M12 5v14M5 12h14" />
                                </svg>
                                লিংক যোগ করুন
                        ...
                        <ul id="footerMenuList" class="mt-3 space-y-2">
                            @forelse ($footerMenuItems as $index => $item)
                                <li class="footer-menu-item flex items-center gap-3 rounded-lg border border-hairline bg-white px-4 py-3 transition-colors hover:border-brand/40" data-index="{{ $index }}">
                                    <span class="sortable-handle cursor-grab text-faint hover:text-ink">
                                        <svg class="h-5 w-5" viewBox="0 0 24 24" fill="currentColor">
                                            <path
                                                d="M8 6a2 2 0 1 1 0 4 2 2 0 0 1 0-4zm0 6a2 2 0 1 1 0 4 2 2 0 0 1 0-4zm0 6a2 2 0 1 1 0 4 2 2 0 0 1 0-4zm8-14a2 2 0 1 1 0 4 2 2 0 0 1 0-4zm0 6a2 2 0 1 1 0 4 2 2 0 0 1 0-4zm0 6a2 2 0 1 1 0 4 2 2 0 0 1 0-4z" />
                                        </svg>
                                    </span>
                                    <input type="text"
                                        class="footer-menu-label flex-1 rounded-md border border-hairline bg-gray-50/50 px-3 py-1.5 text-sm text-ink outline-none transition-colors focus:border-brand focus:bg-white focus:ring-1 focus:ring-brand/20"
                                        value="{{ $item['label'] ?? '' }}" placeholder="মেনু টেক্সট">
                                    <input type="text"
                                        class="footer-menu-url w-40 rounded-md border border-hairline bg-gray-50/50 px-3 py-1.5 text-sm text-ink outline-none transition-colors focus:border-brand focus:bg-white focus:ring-1 focus:ring-brand/20"
                    ...
                    <div class="mt-6">
                        ...
                        <input type="text" id="footer_copyright" name="footer_copyright" value="{{ old('footer_copyright', $settings['footer_copyright'] ?? '') }}"
                            class="w-full rounded-lg border border-hairline bg-gray-50/50 px-4 py-2.5 text-sm text-ink outline-none transition-colors focus:border-brand focus:bg-white focus:ring-1 focus:ring-brand/20"
                            placeholder="© ২০২৬ উসুলি। সর্বস্বত্ব সংরক্ষিত।">
            ...
            $(function() {
                ...
                if ($('#navbarList').length) {
                    ...
                    $.each(protectedTypes, function(i, p) {
                        if (existingTypes.indexOf(p.type) === -1) {
                            var html = '<li class="navbar-item flex items-center gap-3 rounded-lg border border-hairline bg-white px-4 py-3 transition-colors hover:border-brand/40" data-type="' + p.type + '" data-category-id="">' +
                                ...
                                '<input type="text" class="navbar-label flex-1 rounded-md border border-hairline bg-gray-50/50 px-3 py-1.5 text-sm text-ink outline-none transition-colors focus:border-brand focus:bg-white focus:ring-1 focus:ring-brand/20" value="' +
                                p.label + '">' +
                            ...
                            $.get(catSearchUrl, {
                                ...
                                if (data.length === 0) {
                                    ...
                                    $.each(data, function(i, cat) {
                                        $results.append(
                                            '<div class="flex items-center gap-3 px-4 py-3 cursor-pointer transition-colors hover:bg-gray-50 cat-select-item" data-id="' + cat.id + '" data-name="' + cat.name
                                            .replace(/"/g, '&quot;') + '">' +
                                            '<span class="shrink-0 rounded-md bg-purple-50 px-2 py-0.5 text-xs font-medium text-purple-700">ক্যাটাগরি</span>' +
                    ...
                    $('#catSearchResults').on('click', '.cat-select-item', function() {
                        ...
                        var html = '<li class="navbar-item flex items-center gap-3 rounded-lg border border-hairline bg-white px-4 py-3 transition-colors hover:border-brand/40" data-type="category" data-category-id="' + id +
                            '" data-index="' + index + '">' +
                            '<span class="sortable-handle cursor-grab text-faint hover:text-ink"><svg class="h-5 w-5" viewBox="0 0 24 24" fill="currentColor"><path d="M8 6a2 2 0 1 1 0 4 2 2 0 0 1 0-4zm0 6a2 2 0 1 1 0 4 2 2 0 0 1 0-4zm0 6a2 2 0 1 1 0 4 2 2 0 0 1 0-4zm8-14a2 2 0 1 1 0 4 2 2 0 0 1 0-4zm0 6a2 2 0 1 1 0 4 2 2 0 0 1 0-4zm0 6a2 2 0 1 1 0 4 2 2 0 0 1 0-4z"/></svg></span>' +
                            '<span class="shrink-0 rounded-md bg-purple-50 px-2 py-0.5 text-xs font-medium text-purple-700">ক্যাটাগরি</span>' +
                            '<input type="text" class="navbar-label flex-1 rounded-md border border-hairline bg-gray-50/50 px-3 py-1.5 text-sm text-ink outline-none transition-colors focus:border-brand focus:bg-white focus:ring-1 focus:ring-brand/20" value="' +
                ...
                function initPostSearch(inputId, resultsId, hiddenId, selectedId, searchId) {
                            ...
                            $.get(searchUrl, {
                                ...
                                if (data.length === 0) {
                                    ...
                                    $.each(data, function(i, post) {
                                        ...
                                        var html = '<div class="flex items-center gap-3 px-4 py-3 cursor-pointer transition-colors hover:bg-gray-50 post-select-item" data-id="' + post.id + '" data-title="' + post.title
                                            .replace(/"/g, '&quot;') + '" data-image="' + (post.image || '') + '" data-category="' + (post.excerpt || '') + '">' + img +
                                            '<div class="flex-1 min-w-0"><p class="truncate text-sm font-medium text-ink">' + post.title + '</p></div></div>';
                                        $results.append(html);
                                    });
                                }
                                $results.removeClass('hidden');
                    ...
                    $results.on('click', '.post-select-item', function() {
                        ...
                        var imgHtml = image ?
                            ...
                            '<span class="grid h-16 w-16 shrink-0 place-items-center rounded-lg bg-gray-200 text-xs text-faint">ছবি নেই</span>';
                        var cardHtml = '<div class="flex items-center gap-4 rounded-lg border border-brand/30 bg-brand-soft/30 p-4">' + imgHtml + '<div class="flex-1 min-w-0"><p class="truncate font-medium text-ink">' + title +
                            '</p></div><button type="button" onclick="clear' + (inputId === 'heroSearchInput' ? 'Hero' : 'Feature') +
                            'Post()" class="shrink-0 rounded-lg p-1.5 text-faint transition-colors hover:bg-red-50 hover:text-red-500"><svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 6 6 18M6 6l12 12"/></svg></button></div>';
                ...
                if ($('#footerMenuList').length) {
                    ...
                    $('#saveFooterLink').on('click', function() {
                        ...
                        var html = '<li class="footer-menu-item flex items-center gap-3 rounded-lg border border-hairline bg-white px-4 py-3 transition-colors hover:border-brand/40" data-index="' + index + '">' +
                            '<span class="sortable-handle cursor-grab text-faint hover:text-ink"><svg class="h-5 w-5" viewBox="0 0 24 24" fill="currentColor"><path d="M8 6a2 2 0 1 1 0 4 2 2 0 0 1 0-4zm0 6a2 2 0 1 1 0 4 2 2 0 0 1 0-4zm0 6a2 2 0 1 1 0 4 2 2 0 0 1 0-4zm8-14a2 2 0 1 1 0 4 2 2 0 0 1 0-4zm0 6a2 2 0 1 1 0 4 2 2 0 0 1 0-4zm0 6a2 2 0 1 1 0 4 2 2 0 0 1 0-4z"/></svg></span>' +
                            '<input type="text" class="footer-menu-label flex-1 rounded-md border border-hairline bg-gray-50/50 px-3 py-1.5 text-sm text-ink outline-none transition-colors focus:border-brand focus:bg-white focus:ring-1 focus:ring-brand/20" value="' +
                            label.replace(/"/g, '&quot;') + '">' +
                            '<input type="text" class="footer-menu-url w-40 rounded-md border border-hairline bg-gray-50/50 px-3 py-1.5 text-sm text-ink outline-none transition-colors focus:border-brand focus:bg-white focus:ring-1 focus:ring-brand/20" value="' +
                            url.replace(/"/g, '&quot;') + '">' +
