    ...
    <div class="space-y-6">
        ...
        <form method="GET" class="flex flex-wrap items-end gap-4 rounded-xl border border-hairline bg-white p-4">
            <div class="flex-1 min-w-[200px]">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="নাম, বিষয়, ইমেইল বা বার্তা..."
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('admin.contacts.index') }}" class="rounded-lg border border-hairline px-4 py-2.5 text-sm font-medium text-faint transition-colors hover:bg-gray-50">
                    রিসেট
                </a>
            </div>
        </form>
            ...
            <table class="w-full text-left text-sm">
                <thead>
                    <tr class="border-b border-hairline bg-gray-50/50">
                        ...
                        <th class="px-4 py-3 font-medium text-ink">ফোন</th>
                    ...
                    @forelse ($contacts as $contact)
                        <tr class="transition-colors hover:bg-gray-50/50">
                            ...
                            <td class="max-w-[200px] px-4 py-3">
                                <p class="truncate text-faint">{{ $contact->subject }}</p>
                            </td>
                            <td class="px-4 py-3 text-faint">{{ $contact->email ?? '—' }}</td>
                            <td class="px-4 py-3 text-faint">{{ $contact->phone ?? '—' }}</td>
                            <td class="px-4 py-3">
                                @if ($contact->is_seen)
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="flex items-center justify-end gap-1">
                                    <a href="{{ route('admin.contacts.show', $contact) }}" class="rounded-lg p-2 text-faint transition-colors hover:bg-brand/10 hover:text-brand" title="দেখুন">
