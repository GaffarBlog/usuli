                            ...
                            <div id="reply-form-{{ $comment->id }}" class="hidden mt-4 rounded-lg border border-hairline bg-gray-50/50 p-4">
                                <form method="POST" action="{{ route('blog.comment.store', $post->slug) }}">
                                    ...
                                    <div class="mt-3 flex justify-end gap-2">
                                        <button type="button" onclick="document.getElementById('reply-form-{{ $comment->id }}').classList.add('hidden')" class="rounded-lg border border-hairline px-4 py-2 text-xs font-medium text-faint transition-colors hover:bg-gray-100">
                                            বাতিল
                                        </button>
