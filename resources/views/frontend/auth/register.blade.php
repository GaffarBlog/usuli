...
<head>
    ...
    <title>নিবন্ধন — উসুলি</title>
    <meta name="description" content="উসুলিতে একটি অ্যাকাউন্ট তৈরি করুন।">
        ...
        <div class="w-full max-w-[440px]">
            ...
            <div class="rounded-[14px] border border-hairline bg-white px-[clamp(24px,5vw,40px)] py-[clamp(32px,5vw,48px)]">
                ...
                <p class="mb-8 text-center text-[0.98rem] text-faint">
                    উসুলিতে একটি নতুন অ্যাকাউন্ট খুলুন
                </p>
                ...
                <form method="POST" action="{{ route('frontend.register.post') }}">
                    ...
                    {{-- Name --}}
                    <div class="mb-5">
                        <label for="name" class="mb-2 block text-[0.9rem] font-medium text-ink">নাম</label>
                        <input id="name" type="text" name="name" value="{{ old('name') }}" required autofocus autocomplete="name" placeholder="আপনার নাম"
                    ...
                    <div class="mb-5">
                        ...
                        <input id="email" type="email" name="email" value="{{ old('email') }}" required autocomplete="email" placeholder="আপনার ইমেইল"
                            class="w-full rounded-full border border-brand-line bg-[#fafaf8] px-5 py-3.5 text-base text-body outline-none transition-[border-color,box-shadow] duration-300 placeholder:text-faint focus:border-brand focus:ring-3 focus:ring-brand/15">
                        @error('email')
                            <p class="mt-1.5 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="mb-5">
                        ...
                        <input id="password" type="password" name="password" required autocomplete="new-password" placeholder="কমপক্ষে ৮ অক্ষর"
                            class="w-full rounded-full border border-brand-line bg-[#fafaf8] px-5 py-3.5 text-base text-body outline-none transition-[border-color,box-shadow] duration-300 placeholder:text-faint focus:border-brand focus:ring-3 focus:ring-brand/15">
                        @error('password')
                            <p class="mt-1.5 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    {{-- Confirm Password --}}
                    <div class="mb-8">
                        <label for="password_confirmation" class="mb-2 block text-[0.9rem] font-medium text-ink">পাসওয়ার্ড নিশ্চিত করুন</label>
                        <input id="password_confirmation" type="password" name="password_confirmation" required autocomplete="new-password" placeholder="আবার পাসওয়ার্ড লিখুন"
                            class="w-full rounded-full border border-brand-line bg-[#fafaf8] px-5 py-3.5 text-base text-body outline-none transition-[border-color,box-shadow] duration-300 placeholder:text-faint focus:border-brand focus:ring-3 focus:ring-brand/15">
                    </div>
                ...
                <p class="text-center text-[0.98rem] text-body">
                    ...
                    <a href="{{ route('frontend.login') }}" class="font-semibold text-brand-deep transition-colors duration-300 hover:text-brand">
                        প্রবেশ করুন
                    </a>
                </p>
            </div>
