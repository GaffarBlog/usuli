...
<body class="relative bg-page font-sans text-[17px] leading-[1.75] text-body antialiased max-[620px]:text-base">
    @php
        $navLink = 'relative py-1.5 text-[0.98rem] font-medium text-body transition-colors duration-300 after:absolute after:inset-x-0 after:bottom-0 after:h-0.5 after:origin-left after:scale-x-0 after:bg-brand after:transition-transform after:duration-300 hover:text-brand-deep hover:after:scale-x-100 focus-visible:after:scale-x-100';
        $navLinkActive = 'relative py-1.5 text-[0.98rem] font-semibold text-ink transition-colors duration-300 after:absolute after:inset-x-0 after:bottom-0 after:h-0.5 after:origin-left after:scale-x-100 after:bg-ink after:transition-transform after:duration-300';
    ...
    <a href="#main"
       class="absolute left-4 top-[-60px] z-[200] rounded-lg bg-brand px-[18px] py-2.5 font-semibold text-white transition-[top] duration-300 focus:top-3.5">
        মূল অংশে যান
    </a>
    ...
    <header class="sticky top-0 z-100 border-b border-hairline bg-[rgba(250,250,248,0.82)] backdrop-blur-md backdrop-saturate-[140%]">
        <div class="shell flex h-[76px] items-center justify-between gap-6">
            <a href="{{ route('home') }}" aria-label="উসুলি — প্রচ্ছদ" class="inline-flex items-center gap-3">
                <svg class="h-[45px] w-[30px] shrink-0 overflow-visible" viewBox="0 0 60 90" aria-hidden="true" focusable="false">
                    ...
                    <path class="opacity-0 animate-fade-in-late motion-reduce:animate-none motion-reduce:opacity-100"
                          d="{{ $brandAccentPath }}"
                          fill="none" stroke="#2b8ca4" stroke-width="4" stroke-linecap="round"/>
                </svg>
            ...
            <nav class="mx-auto hidden min-[1001px]:block" aria-label="প্রধান নেভিগেশন">
                <ul class="flex gap-[30px]">
                    @foreach ($navItems as $item)
                        <li>
                            <a href="{{ $item['href'] ?? '#' }}"
                               class="{{ ($item['label'] === ($activeNav ?? '')) ? $navLinkActive : $navLink }}">{{ $item['label'] }}</a>
                        </li>
                    @endforeach
                </ul>
            </nav>
            ...
            <div class="flex items-center gap-1.5">
                <button type="button" aria-label="খুঁজুন"
                        class="grid h-[42px] w-[42px] place-items-center rounded-full text-ink transition-colors duration-300 hover:bg-brand-soft hover:text-brand-deep">
            ...
            <div class="flex gap-3 pt-1.5" aria-label="সামাজিক মাধ্যম">
                <a href="#" aria-label="ফেসবুক"
                   class="grid h-10 w-10 place-items-center rounded-full border border-white/15 text-footer-mut transition-colors duration-300 hover:border-[#4fb6cf] hover:bg-[#4fb6cf]/12 hover:text-white">
                    <svg class="h-[19px] w-[19px]" viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linejoin="round"><path d="M14 9V7c0-1 .3-1.5 1.6-1.5H17V2.5h-2.4C11.9 2.5 11 4 11 6.3V9H8.5v3H11v9.5h3V12h2.2l.4-3H14z"/></svg>
                </a>
                <a href="#" aria-label="এক্স"
                   class="grid h-10 w-10 place-items-center rounded-full border border-white/15 text-footer-mut transition-colors duration-300 hover:border-[#4fb6cf] hover:bg-[#4fb6cf]/12 hover:text-white">
                    <svg class="h-[19px] w-[19px]" viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linejoin="round"><path d="M17.5 3h3l-6.6 7.5L21.7 21h-5.9l-4.3-5.6L6.4 21H3.3l7-8L2.6 3h6l3.9 5.1L17.5 3zm-1 16h1.6L7.6 4.6H5.9L16.5 19z"/></svg>
                </a>
                <a href="#" aria-label="ইনস্টাগ্রাম"
                   class="grid h-10 w-10 place-items-center rounded-full border border-white/15 text-footer-mut transition-colors duration-300 hover:border-[#4fb6cf] hover:bg-[#4fb6cf]/12 hover:text-white">
                    <svg class="h-[19px] w-[19px]" viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linejoin="round"><rect x="3.5" y="3.5" width="17" height="17" rx="5"/><circle cx="12" cy="12" r="4"/><circle cx="17.2" cy="6.8" r="1.2" fill="currentColor" stroke="none"/></svg>
                </a>
                <a href="#" aria-label="ইউটিউব"
                   class="grid h-10 w-10 place-items-center rounded-full border border-white/15 text-footer-mut transition-colors duration-300 hover:border-[#4fb6cf] hover:bg-[#4fb6cf]/12 hover:text-white">
                    <svg class="h-[19px] w-[19px]" viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linejoin="round"><rect x="2.5" y="5.5" width="19" height="13" rx="4"/><path d="M10 9.2l5 2.8-5 2.8V9.2z" fill="currentColor" stroke="none"/></svg>
                </a>
            </div>
        </div>
